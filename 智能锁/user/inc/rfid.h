...
#define     DivIrqReg             0x05    //中断请求标志位
#define     ErrorReg              0x06    //错误标志，指示执行的上个命令的错误状态 
#define     Status1Reg            0x07    
#define     Status2Reg            0x08     //接收器和发送器的状态标志位
#define     FIFODataReg           0x09     //64字节FIFO缓冲区的输入和输出
#define     FIFOLevelReg          0x0A     //FIFO中存储的字节数 
...
#define     ModeReg               0x11  //定义发送接收的常用模式
#define     TxModeReg             0x12
#define     RxModeReg             0x13
#define     TxControlReg          0x14  //控制天线驱动器的管脚TX1和TX2
...
#define  CS_H  GPIOA->ODR|=1<<6
#define CS_L  GPIOA->ODR&=~(1<<6)

#define  RST_H  GPIOA->ODR|=1<<7
#define RST_L  GPIOA->ODR&=~(1<<7)
...
char PcdComMF522(u8   Command, u8 *pIn , u8   InLenByte,u8 *pOut , u8  *pOutLenBit);									//功    能：通过RC522和ISO14443卡通讯
...
char PcdAuthState(u8 auth_mode,u8   addr,u8 *pKey,u8 *pSnr);    //验证卡密
